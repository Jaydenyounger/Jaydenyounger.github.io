---
title: "AEGIS"
excerpt: "Condemned by the gods. Bound to fight forever."

header:
  overlay_image: /assets/images/AEGIS Title.jpg
  teaser: /assets/images/AEGIS Title.jpg
  overlay_filter: 0.5

project_brief: "First person shield basher"
project_role: "Team Lead and Game Designer"
project_tools: "Unreal Engine version 5.4.4, Blender, Substance Painter"
project_date: "April 7, 2026"

---

## About
*Condemned by the gods. Bound to fight forever.*

Play as a Spartan warrior cursed by Olympus, trapped in an endless cycle of divine punishment. Armed with nothing but a living shield, you must survive whatever the gods unleash.

Aegis is an infinite FPS arena built on precision, momentum, and controlled aggression. Your shield isn’t just defense, it’s your weapon. Charge it, ricochet it off walls, and tear through enemies as each round grows faster, deadlier, and more relentless.

The enemies are ruthless as they are unyielding, but you are equally so.

You are fast. You are powerful. You are relentless. Move with precision. Strike with force. Dominate through momentum.

---

## Development

[Development log: 1](https://jaydenyounger.itch.io/aegis/devlog/1472542/development-day-1)

> Initalized the basic project and had our most basic systems and Shield tracing path method guid the shield.


[Development log: 2](https://jaydenyounger.itch.io/aegis/devlog/1472545/devlopement-day-2)

> Improved player movement and fixed shield pathing bugs by tracing the last impact point


[Development log: 3](https://jaydenyounger.itch.io/aegis/devlog/1473419/development-day-3)

> Implemented a health system, Implemented the Kinetic system, and added Shield bashing.

> Trace bug fix where we could not get the absolute last point when the last trace did not hit anything. Had to create a condition where if the trace did not hit, make the impact point go to the location of the trace end instead of the impact point. 


[Development log: 4](https://jaydenyounger.itch.io/aegis/devlog/1474285/development-day-4)

> Added basic AI and enemy death

> The player can now block attacks

> Improvements to the shield bash feature


[Development log: 6](https://jaydenyounger.itch.io/aegis/devlog/1478693/development-day-6)

> The shield throw has been implemented and added to the game.

> Improved movement with shield bashing

> The focus state has been fully realized.

> A rough layout for the main level has been made


[Development log: 7](https://jaydenyounger.itch.io/aegis/devlog/1479681/development-day-7)

> All game systems are in working order. 


[Development log: 8](https://jaydenyounger.itch.io/aegis/devlog/1481529/development-day-8)

> Work on the visual elements has also begun.

> Had a multiple merge issues with porting assets into the main branch on Diversion. We manage to fixed this issue by branching off and cleaning files, but this did cause a halt in level development and design.


[Development log: 9](https://jaydenyounger.itch.io/aegis/devlog/1481518/development-day-9)

> Refining and developing our main level


[Development log: 10](https://jaydenyounger.itch.io/aegis/devlog/1482885/development-day-10)

> We are in the development point where we had extra time to develop and add more to our game like upgrades. 


[Development log: 11](https://jaydenyounger.itch.io/aegis/devlog/1485008/development-day-11)

> The game has been fully released and is up on the Unlikely Collaborator game jam.




## Developers
- [Team lead: Jayden Younger](https://jaydenyounger.itch.io)
- [AI programmer: Connor Touchet](  https://hollowedgenos.itch.io/)
- [Level Designer: Alex Homeyer](https://alexhomeyer.itch.io/)
- [General level and 3D Artist: Cormac Bulmer](https://damp-weasel.itch.io/)
- [Game Artist: Erielle Seigel](https://www.artstation.com/erielleseigel)
- [Game Artist: Annette Lau](https://www.artstation.com/net-exe)

---

## Development

### Resources
- [Download the game here](https://jaydenyounger.itch.io/aegis)
- [Gameplay footage](https://youtu.be/D95gUxiRr3w?si=FoPsNbf7c842giZY)
- [Unlikely Collaborator game jam](https://itch.io/jam/ucgamejam2026/rate/4417834)